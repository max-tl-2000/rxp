...
export interface IFeatureBranchDescriptor {
  branch: string;
  base: string;
}
...
export interface IMergeBranchesArgs {
  ...
  dryRun: boolean;
  branches: string[];
  featureBranches: IFeatureBranchDescriptor[];
...
export const getCheckForMergeMessageCommand = (args: ICheckForMergeMessageArgs): string =>
  `git log | grep ${args.mergeMessage} || true`;
...
export const getCurrentBranch = async () => {
  const branch = await execp('git rev-parse --abbrev-ref HEAD');
  return trim(branch);
};

export const changeToBranch = async (branch: string) => doExec(`git checkout ${branch}`);

export const fetchFromRemote = async (remote: string) => doExec(`git fetch ${remote}`);
