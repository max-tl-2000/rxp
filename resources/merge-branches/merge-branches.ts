...
export const getCommandsToExecutePerBranch = (
  branches: string[],
  remote: string,
  ...
  branches.reduce((acc: ICommandsPerBranch[], branch, index) => {
    ...
    const validateTasks = getValidationTasks({
      commitsToVerify,
      branches,
    ...
    if (prevBranch) {
      commands.push({
        cmd: `git merge ${prevBranch} --no-edit --no-ff`,
        description: `Merging ${prevBranch} into ${branch}`,
...
const getCommand = (args: ICommand, cmdPrefix: string, addComment = false): string => {
  let { cmd } = args;
...
const printCommandsNotYetExecuted = (
  commandsPerBranch: ICommandsPerBranch[],
  cmdPrefix: string,
  options: ICommandOptions,
) => {
  const allMissingCommands = commandsPerBranch.reduce((acc: ICommand[], cPerBranch) => {
    acc = acc.concat(cPerBranch.getNotExecutedCommands(options)); // eslint-disable-line no-param-reassign
    return acc;
  }, []);
    ...
    (acc, command: ICommand): Promise<void> =>
      acc.then(async () => {
        ...
        if (typeof validateFn === 'function') {
          await validateFn();
          command.executed = true; // eslint-disable-line no-param-reassign
          return;
        }
...
export const mergeBranches = async (args: MergeBranchesArgs): Promise<void> => {
  ...
  subtle('options', { dryRun, pull, merge, push, remote, validate, cmdPrefix, branches, featureBranches });

  let commandsPerBranch = getCommandsToExecutePerBranch(branches, remote, commitsToVerify, ignoreMergeCheckOnBranches);

  featureBranches.forEach(featureBranch => {
    const { branch, base } = featureBranch;
    commandsPerBranch = [...commandsPerBranch, ...getCommandsToExecutePerBranch([base, branch], remote, [], [])];
  });
